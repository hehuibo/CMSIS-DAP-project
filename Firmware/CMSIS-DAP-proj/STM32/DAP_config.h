...
// USB Connect Pull-Up
	...
	#define PIN_USB_CONNECT_PIN		10
	#define PIN_USB_CONNECT         PIN_MASK(PIN_USB_CONNECT_PIN)
	#define PIN_USB_MODE			GPIO_Mode_Out_PP//GPIO_Mode_Out_OD
...
__STATIC_INLINE  uint32_t PIN_SWCLK_TCK_IN  (void) {
  return (PIN_SWCLK_TCK_PORT->ODR & PIN_SWCLK_TCK) ? 1U : 0U;
}
...
__STATIC_INLINE uint32_t PIN_SWDIO_TMS_IN  (void) {
  return (PIN_SWDIO_TMS_PORT->IDR & PIN_SWDIO_TMS) ? 1U : 0U;
}
...
__STATIC_INLINE uint32_t PIN_TDI_IN  (void) {
    return (PIN_TDI_PORT->ODR & PIN_TDI) ? (1U) : (0U);
}
...
__STATIC_INLINE uint32_t PIN_TDO_IN  (void) {
  return (PIN_TDO_PORT->IDR & PIN_TDO) ? (1U) : (0U);
}
#endif
