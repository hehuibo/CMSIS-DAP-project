...
static uint32_t DAP_SWJ_Pins(const uint8_t *request, uint8_t *response) {
  ...
  if (select & (1U << DAP_SWJ_TDI)) {
	...
	//----------------进行软件复位 -------------------------//
	Reset_Count ++ ;
	if(	Reset_Count>0	) //第二次进入才复位
	{
		Reset_Count = 0;
		write_data = 0xe000ed0c;SWD_Transfer(0x05,&write_data);Delayms(1);	// 往TAR写入 SCB->AIRCR 地址
		write_data = 0x05fa0004;SWD_Transfer(0x0d,&write_data);Delayms(1);	// 往DRW写入 目标设计
	}
  #endif
  //----------------硬件复位-------------------//
